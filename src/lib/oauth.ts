...
export async function signInWithOAuth(provider: OAuthProvider): Promise<void> {
  // expo-router's deep link target — an unmatched route is fine, the redirect
  // is intercepted by WebBrowser before it ever reaches the router.
  ...
  throw new Error(errorDescription ?? 'Sign-in did not return a session.');
}
